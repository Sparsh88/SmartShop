    ...
    <div className="flex flex-col md:flex-row min-h-[calc(100vh-4rem)]">
      ...
      <main className="flex-1 p-6 sm:p-8 space-y-8 bg-slate-950 animate-page-enter">
        ...
        {isLoading ? (
          ...
          <>
            {/* KPI Cards Grid - Staggered */}
            <ScrollRevealGroup staggerDelay={0.1} delayChildren={0.05} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <ScrollReveal direction="up" distance={30} duration={0.6}>
                <div className="bg-slate-900 border border-slate-850 p-6 rounded-2xl space-y-4 shadow-sm">
                  <h3 className="text-lg font-bold text-white flex items-center gap-2">
                    <AlertTriangle size={18} className="text-amber-400" /> Critical Stock Warnings
                  </h3>

                  {lowStockProducts.length === 0 ? (
                    <p className="text-slate-500 text-sm">All products have sufficient inventory.</p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-xs border-collapse">
                        <thead>
                        ...
                        <tbody className="divide-y divide-slate-850">
                          {lowStockProducts.map((prod: any) => (
                            <tr key={prod.id}>
                              ...
                              <td className="py-3 text-slate-400">{prod.brand}</td>
                              <td className="py-3 text-right font-bold text-rose-400">{prod.stock} units</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
